...
main (int argc, char *argv[])
{
  ...
  std::string outputDir = "/home/manee/Downloads/nrRepo/ns-3-dev/scratch/nr-v2x-examples-broadcast/";
  ...
  std::uniform_int_distribution<int> distribution (0, ueNum);
  ...
  for (uint16_t i = 0; i < ueNum; i++)
    {
      if (i != randomValue)
        {
          AddressValue remoteAddres (InetSocketAddress (ueIpIface.GetAddress (i), port));
          onoff1.SetAttribute ("Remote", remoteAddres);

          app = onoff1.Install (ueVoiceContainer.Get (randomValue));
          app.Start (Seconds (3 + jitter));
  ...
  monitor->SerializeToXmlFile ("broadcastV2xFlow.xml", true, true);
...
for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i) {
      std::cout << "  Average Tx Packets:" << iter->second.rxBytes << std::endl;
      std::cout << "  Average Tx Bytes:" << iter->second.rxBytes << std::endl;
      std::cout << "  Average Rx Packets:" << iter->second.rxBytes << std::endl;
      std::cout << "  Average Rx Bytes:" << iter->second.rxBytes << std::endl;
      std::cout << "  Average Throughput:" << iter->second.rxBytes << std::endl;
      double averageFlowDuration = totalFlowDuration / stats.size();
      std::cout << "  Average Delay:" << iter->second.rxBytes << std::endl;
...
for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i) {
    ...
    // Add flow throughput to total throughput
  ...
  std::cout << "  Average Throughput:" << iterator->second.rxBytes << std::endl;
*/
  /*
