...
main (int argc, char *argv[])
{
  ...
  std::string outputDir = "/home/manee/Downloads/nrRepo/ns-3-dev/scratch/nr-v2x-examples-groupcast/";
...
//numGroups = ueNum / 4; // 1 transmitter and 3 receivers per group

std::vector<std::vector<int>> groupcastGroups;
std::set<int> availableUeIds;



// Initialize available user IDs
for (uint16_t i = 0; i < ueNum; ++i) {
    availableUeIds.insert(static_cast<int>(i));
}

std::random_device rd;
std::mt19937 gen(rd());

    while (!availableUeIds.empty()) {
        std::vector<int> group;

        // Generate a random group size between 1 and the remaining available UEs
        int maxGroupSize = std::min(10, static_cast<int>(availableUeIds.size()));
    std::uniform_int_distribution<int> groupSizeDistribution(2, maxGroupSize);
        int groupSize = groupSizeDistribution(gen);

if (groupSize >= 2) {
        for (uint16_t k = 0; k < groupSize; ++k) {
            std::uniform_int_distribution<int> ueDistribution(0, static_cast<int>(availableUeIds.size()) - 1);
            int ueId = *std::next(std::begin(availableUeIds), ueDistribution(gen));
            availableUeIds.erase(ueId);
            group.push_back(ueId);
        }

        groupcastGroups.push_back(group);
    }else{
// Group size less than 2, consider these vehicles as dropped
            for (uint16_t k = 0; k < groupSize; ++k) {
                std::uniform_int_distribution<int> ueDistribution(0, static_cast<int>(availableUeIds.size()) - 1);
                int ueId = *std::next(std::begin(availableUeIds), ueDistribution(gen));
                availableUeIds.erase(ueId);
    ...
    // Print groupcast groups
    std::cout << "Groupcast Groups:" << std::endl;
    for (uint16_t i = 0; i < groupcastGroups.size(); ++i) {
        std::cout << "Group " << i + 1 << ": ";
        for (uint16_t k = 0; k < groupcastGroups[i].size(); ++k) {
            std::cout << "(" << groupcastGroups[i][k] << ") ";
        }
        std::cout << std::endl;
        
        
        //std::cout << "Dropped Vehicle IDs: None"<< std::endl;
    }

    // Print dropped (unassigned) vehicles
    /*std::cout << "Dropped Vehicle IDs: ";
    for (const int& ueId : availableUeIds) {
        std::cout << "("<< ueId << ") ";
    }
    std::cout << std::endl;*/
 ...
 // Create and configure the OnOff application
    OnOffHelper onoff2 ("ns3::UdpSocketFactory", Address ());
    std::string dataRateBeString = std::to_string (dataRateBe) + "kb/s";
    onoff2.SetConstantRate (DataRate (dataRateBeString));
    numGroups = groupcastGroups.size();
    for (unsigned int i = 0; i < numGroups; ++i) {
        // Get the transmitter's mobility model
        Ptr<MobilityModel> mobiModltx = ueVoiceContainer.Get (groupcastGroups[i][0])->GetObject<MobilityModel> ();
        
        ApplicationContainer groupApps;
           
        // Set the remote address to the receivers in the group
        // Set the remote address to the receivers in the group
for (int k = 1; k < groupcastGroups[i].size(); ++k) {
// Get the receiver's mobility model
    Ptr<MobilityModel> mobiModlrx = ueVoiceContainer.Get(groupcastGroups[i][k])->GetObject<MobilityModel>();
    double distance = mobiModltx->GetDistanceFrom(mobiModlrx);

// Set the remote address to the receiver's address
          Ipv4Address txAdrs = ueVoiceContainer.Get(groupcastGroups[i][0])
            ->GetObject<Ipv4L3Protocol>()
            ->GetAddress(1, 0)
            .GetLocal();
            
            Ipv4Address rxAdrs = ueVoiceContainer.Get(groupcastGroups[i][k])
            ->GetObject<Ipv4L3Protocol>()
            ->GetAddress(1, 0)
            .GetLocal();
      ...
      std::cout << "  (Tx Vehicle Num : " << groupcastGroups[i][0] << " - Tx Address : " << txAdrs
                << ") --> (Rx Vehicle Num : " << groupcastGroups[i][k] << " - Rx Address : " << rxAdrs
                << ") => Distance : " << distance << " meters" << std::endl;    
                
        
        
        AddressValue remoteAddress(InetSocketAddress(rxAdrs, port));
        onoff2.SetAttribute("Remote", remoteAddress);
        
        ApplicationContainer app = onoff2.Install(ueVoiceContainer.Get(groupcastGroups[i][0])); // Transmitter
...
groupApps.Add(app);
    ...
    double groupStartTime = 3 + i + jitter; // Adjust timing as needed
      groupApps.Start (Seconds (groupStartTime));
      groupApps.Stop (Seconds (Now ().GetSeconds ()));
    }
  //}
  ...
  monitor->SerializeToXmlFile ("GroupcastV2xFlow.xml", true, true);
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin ();
       iter != stats.end (); ++iter)
    {
      ...
      std::cout << "Group: " << t.sourceAddress << " -> " << t.destinationAddress << std::endl;
