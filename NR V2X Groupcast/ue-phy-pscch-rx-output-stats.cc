  ...
  for (const auto & v : m_pscchCache)
    {
      ...
      NS_ABORT_UNLESS (ret);
      if (!std::isnan(v.m_sinr)) {
    // v.m_sinr is valid, insert it into the database
    ret = m_db->Bind(stmt, 13, v.m_sinr);
    NS_ABORT_UNLESS(ret);
} else {
    // v.m_sinr is NULL or invalid, provide a default value or handle it as needed
    double defaultSinr = -1.0; // Use -1.0 to represent NULL
    ret = m_db->Bind(stmt, 13, defaultSinr);
    NS_ABORT_UNLESS(ret);
}
